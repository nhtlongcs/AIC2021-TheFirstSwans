...
from vietocr.tool.translate import process_image_v2
...
class OCRDataset(Dataset):
    ...
    def get_bucket(self, idx):
        ...
        new_w, image_height = resize(imgW, imgH, self.image_height, self.image_min_width, self.image_max_width, keep_width=True)
    ...
    def read_data(self, idx):
        ...
        img_bw = process_image_v2(img, self.image_height, self.image_min_width, self.image_max_width)
...
class Collator(object):
    ...
    def __call__(self, batch):
        ...
        max_width = batch[0]['img'].shape[2]
        for sample in batch:
            max_width = max(max_width, sample['img'].shape[2])

        for sample in batch:
            c, h, w = sample['img'].shape
            pad_img = np.zeros((c, h, max_width))
            pad_img[:, :, :w] = sample['img'][:, :, :]
