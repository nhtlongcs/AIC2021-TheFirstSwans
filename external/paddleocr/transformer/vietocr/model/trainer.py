from vietocr.optim.optim import ScheduledOptim, CosineWithRestarts
...
from vietocr.loggers.logger import Logger
from vietocr.postprocess.clean import custom_postprocess
...
class Trainer():
    def __init__(self, config, pretrained=True, augmentor=ImgAugTransform(), use_custom_postproc=True):
        ...
        self.use_custom_postproc = use_custom_postproc

        if not os.path.exists(self.export_weights):
            os.mkdir(self.export_weights)

        self.config.save(os.path.join(self.export_weights, 'config.yml'))
        self.logger = Logger(self.export_weights)
        ...
        self.optimizer = AdamW(self.model.parameters(), betas=(0.9, 0.98), eps=1e-09, lr=config['optimizer']['lr'])
        ...
        self.scheduler = CosineWithRestarts(self.optimizer, T_max=len(self.train_gen))

        if self.checkpoint is not None:
            self.iter, self.best_acc = self.load_checkpoint(self.checkpoint)

        print("Current iteration: ", self.iter)
        print("Current best accuracy: ", self.best_acc)
    ...
    def train(self):
        ...
        for i in range(self.num_iters):
            ...
            if self.iter % self.print_every == 0:
                ...
                print(info)
                self.logger.write(
                    tags=['train/loss', 'train/lr'],
                    values=[total_loss*1.0/self.print_every, self.optimizer.param_groups[0]['lr']],
                    step=self.iter)

            if self.valid_annotation and self.iter % self.valid_every == 0:
                val_loss = self.validate()
                acc_full_seq, acc_per_char = self.precision(self.metrics)
                self.visualize_prediction(logging=True)

                info = 'iter: {:06d} - valid loss: {:.3f} - acc full seq: {:.4f} - acc per char: {:.4f}'.format(self.iter, val_loss, acc_full_seq, acc_per_char)
                
                print(info)
                self.logger.write(
                    tags=['val/loss', 'val/seq_acc', 'val/char_acc'],
                    values=[val_loss, acc_full_seq, acc_per_char],
                    step=self.iter)


                if acc_full_seq > self.best_acc:
                    best_path = os.path.join(self.export_weights, 'best.pth')
                    self.save_weights(best_path)
                    self.best_acc = acc_full_seq

                latest_path = os.path.join(self.export_weights, 'latest.pth')
                self.save_weights(latest_path)
...
#                loss = self.criterion(rearrange(outputs, 'b t v -> (b t) v'), rearrange(tgt_output, 'b o -> (b o)'))
    ...
    def predict(self, sample=None):
        ...
        for batch in  self.valid_gen:
            ...
            if self.use_custom_postproc:
                pred_sent = custom_postprocess(pred_sent)
    ...
    def visualize_prediction(self, sample=16, errorcase=False, fontname='serif', fontsize=16, logging=False):
        ...
        for vis_idx in range(0, len(img_files)):
            ...
            if logging:
                self.logger.write_image(f'val/{savename}', fig, step = self.iter)
