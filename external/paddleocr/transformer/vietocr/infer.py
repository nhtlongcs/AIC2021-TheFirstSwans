...
from vietocr.postprocess.clean import clean_text
...
parser.add_argument('--labels', type=str,
                    help='path to detection annotation folder')
parser.add_argument('--output', type=str,
                    help='path to output ocr annotation folder')
parser.add_argument('--teencode', action='store_true',
                    help='whether to use teen code')
...
def main(predictor, args):
    ...
    for image_name in tqdm(image_names):
        ...
        with open(ann_path, 'r', encoding="utf8") as f:
            ...
            for line in data:
                tokens = line.split(',')
                x1, y1, x2, y2, x3, y3, x4, y4 = list(map(float, tokens[:8]))
        ...
        polygons = crop_box(img, boxes, out_folder=cropped_folder)

        crop_names = [
            str(i)+'.png' for i in range(len(os.listdir(cropped_folder)))]
            ...
            for idx, (crop_name, polygon) in enumerate(zip(crop_names, polygons)):
                ...
                if os.path.exists(crop_path):
                    ...
                    if (idx+1) % BATCH_SIZE == 0 or idx == len(polygons)-1:
                        ...
                        texts, scores = predictor.predict_batch(
                            batch, return_prob=True)
                        ...
                        for text, polygon, score in zip(texts, batch_polygons, scores):
                            ...
                            f.write(','.join([str(int(i))
                                    for i in ann_text]) + ','+text+'\n')
                        batch = []
                        batch_polygons = []
