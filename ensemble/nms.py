...
def soft_nms(boxes_in, Nt_thres=0.3, threshold=0.6, sigma=0.5, method=2):
    """
    soft_nms
    :para boxes_in, N x 9 (coords + score)
    :para threshold, eliminate cases min score(0.001)
...
def nms_locality(polys, Nt_thres=0.3, **kwargs):
    """
...
fuse_methods_factory = {
    "nms_locality": nms_locality,  # locality aware nms
    "soft_nms": soft_nms,  # soft nms
    "nms": nms,  # nms
    "standard_nms": standard_nms,  # standard nms
}


if __name__ == "__main__":
    # dummy N * 9 numpy array for testing
    boxes = np.array(
        [
            [343, 350, 448, 135, 474, 143, 369, 359, 0.9],
            [343, 350, 448, 135, 474, 143, 369, 340, 0.8],
        ]
    )
    print(boxes.shape)
    print(soft_nms(boxes, 0.3))


# 1 trainval(pp + mask)
# 2 Kfold(pp + mask) -> fuse 3 pp, fuse 3 mask(eval val)
# 3 fuse pp + mask
# nms_weight(a,b, [models])
